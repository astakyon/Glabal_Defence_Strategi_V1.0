...
import { GameState, Threat, ThreatType, Country, Unit, CountryState, UserProfile, SaveGame, CountryMetadata, CountryData } from '../src/types';
import { DataManager } from '../src/services/dataManager';
import type { MapMode, GamePhase } from '../src/types';
import { INITIAL_RESOURCES, THREAT_TYPES, COUNTRY_ISO_MAP, VERSION } from '../src/constants';
import Globe from '../src/Globe';
import EventLogSidebar from '../src/components/EventLogSidebar';
import MainGameArea from '../src/components/MainGameArea';
...
export default function App() {
  ...
  const handleCounterThreat = (threatId: string, type: string, countryId: string) => {
    setGameState(prev => {
      ...
      else if (type === 'İç Karışıklık' || type === 'İç Savaş') { cost = 200; reqSpies = 1; }
      else if (type === 'Suikast') { cost = 50; reqSpies = 1; }
      else if (type === 'Savaş') cost = 300;

      if (prev.resources.budget < cost) return prev;
      if (reqSpies > 0 && (prev.worldState[countryId]?.spies || 0) < reqSpies) return prev;

      return {
        ...prev,
        resources: { ...prev.resources, budget: prev.resources.budget - cost },
        threats: prev.threats.filter(t => t.id !== threatId),
        logs: [`[Başarı] ${type} tehdidi başarıyla bertaraf edildi.`, ...prev.logs]
      };
    });
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 flex flex-col font-sans">
      {appState === 'playing' && (
        <>
          <div className="h-16 bg-slate-900 border-b border-slate-800 flex items-center px-6 justify-between flex-shrink-0">
            <div className="flex items-center gap-4">
              <h1 className="text-xl font-bold text-white tracking-wider">GLOBAL DEFENSE</h1>
              ...
              {playerCountry && (
                <div className="flex items-center bg-slate-800 px-3 py-1 rounded-full border border-slate-700">
                  {getFlag(playerCountry.name, countryMetadata)}
                  <span className="text-sm font-medium text-white">{playerCountry.name}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2 bg-slate-800 px-4 py-1.5 rounded-lg border border-slate-700">
                <span className="text-xs text-slate-400 uppercase font-bold">Tur</span>
                <span className="text-lg font-mono text-white font-bold">{gameState.turn}</span>
              </div>
              <div className="flex items-center gap-2 bg-slate-800 px-4 py-1.5 rounded-lg border border-slate-700">
                ...
                >
                  <option value={4000}>Yavaş</option>
                  <option value={2000}>Normal</option>
                  <option value={1000}>Hızlı</option>
                </select>
              </div>
              <button onClick={() => setAutoPlay(!autoPlay)} className={`p-2 rounded-lg ${autoPlay ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>
                {autoPlay ? <Pause size={20} /> : <Play size={20} />}
              </button>
              <div className="flex items-center gap-4 border-l border-slate-700 pl-6">
                <div className="flex items-center gap-2 text-yellow-500">
                  <Coins size={18} />
                  <span className="font-mono font-bold">{gameState.resources.budget}</span>
                </div>
                <div className="flex items-center gap-2 text-blue-500">
                  <Activity size={18} />
                  <span className="font-mono font-bold">{gameState.resources.intelligence}</span>
                </div>
                <div className="flex items-center gap-2 text-red-500">
                  <Swords size={18} />
                  <span className="font-mono font-bold">{gameState.resources.military}</span>
                </div>
                <div className="flex items-center gap-2 text-green-500">
                  <Shield size={18} />
                  <span className="font-mono font-bold">{gameState.resources.stability}</span>
                </div>
              </div>
              <button onClick={nextTurn} className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2">
      ...
      {appState === 'menu' && (
        <div className="min-h-screen flex items-center justify-center bg-slate-950">
          <div className="text-center space-y-6">
            <h1 className="text-6xl font-black text-white tracking-tighter">GLOBAL DEFENSE</h1>
            <div className="space-y-2">
              <button onClick={() => setAppState('select_country')} className="block w-64 mx-auto bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-full font-bold">Yeni Oyun</button>
              <button onClick={() => setAppState('saves')} className="block w-64 mx-auto bg-slate-800 hover:bg-slate-700 text-white py-3 rounded-full font-bold">Kayıtlı Oyunlar</button>
            </div>
          </div>
        </div>
      )}
      {appState === 'select_country' && (
        <div className="min-h-screen flex items-center justify-center bg-slate-950 p-8">
          <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 w-full max-w-2xl">
            <h2 className="text-2xl font-bold text-white mb-6">Ülke Seç</h2>
            <div className="grid grid-cols-2 gap-4">
              {Object.values(countryMetadata).slice(0, 10).map(c => (
                <button key={c.en} onClick={() => { setPlayerCountry({ id: c.en, name: c.tr }); setAppState('playing'); }} className="bg-slate-800 hover:bg-slate-700 p-4 rounded-xl text-left border border-slate-700">
                  {getFlag(c.tr, countryMetadata)}
                  <span className="font-bold text-white">{c.tr}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      {appState === 'saves' && (
        <div className="min-h-screen flex items-center justify-center bg-slate-950 p-8">
          <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 w-full max-w-2xl">
            <h2 className="text-2xl font-bold text-white mb-6">Kayıtlı Oyunlar</h2>
            <div className="space-y-2">
              {saves.map(s => (
                <button key={s.id} onClick={() => loadGame(s)} className="w-full bg-slate-800 hover:bg-slate-700 p-4 rounded-xl flex justify-between items-center">
                  <span className="text-white font-medium">Tur {s.gameState.turn} - {s.timestamp}</span>
                </button>
              ))}
              <button onClick={() => setAppState('menu')} className="w-full bg-slate-700 hover:bg-slate-600 text-white py-3 rounded-full font-bold mt-4">Geri</button>
            </div>
          </div>
        </div>
      )}
